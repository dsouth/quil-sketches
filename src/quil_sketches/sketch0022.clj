...
(defn draw-triangle [x-centre y-centre x-dia y-dia]
  (let [x-dia-half (int (/ x-dia 2))
        y-dia-half (int (/ y-dia 2))
        x1 (- x-centre x-dia-half)
        y1 (+ y-centre y-dia-half)
        x2 (+ x-centre x-dia-half)
        y2 y1
        x3 x-centre
        y3 (- y-centre y-dia-half)]
    (triangle x1 y1 x2 y2 x3 y3)))

(defn draw-shape [x y x-dia y-dia]
;  (draw-ellipse x y x-dia y-dia)
;  (draw-rect x y x-dia y-dia)
  (draw-triangle x y x-dia y-dia))
